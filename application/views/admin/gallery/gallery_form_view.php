...
$row_image=!empty($this->recent_data['image'])?$this->recent_data['image']:(base_url().'assets/default/no_image_small.png');
    ...
    <?=$this->session->flashdata('pesan')?>
      <div class="box box-info">
        ...
        ?>
              ...
              <div class="form-group <? !empty(form_error('image'))?printf('has-error'):'' ?>">
                <?
          echo form_label('Image','image');
          ?>
          <div class="col-sm-12"><img width="200px" src="<? !empty($row_image)&&file_exists(FCPATH.$row_image)?printf(base_url().$row_image):printf(base_url().'assets/default/no_image_small.png') ?>"></div>
          <?
          echo form_error('image', '<span class="help-block">', '</span>');
        ?>
        <input type="file" name="image" class="form-control" accept="image/*">
              </div>          
              <? /*
              <div class="form-group <? !empty(form_error('content_type_id'))?printf('has-error'):'' ?>">
                <?
